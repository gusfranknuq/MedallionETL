...
def test_unnest_sales_items_handles_missing_items_with_nulls(spark):
    ...
    result = unnest_sales_items(df).select("sale_id", "sku", "quantity", "unit_price")
    rows = result.collect()

    assert len(rows) == 1
    assert rows[0].sale_id == "tx-2"
